...
export default function UnitDialog({ unit, customerId, customers = [], unitBrands, maintenanceTypes, onSave, onClose }) {
  ...
  // Filter maintenance types for this brand (brand-specific + unassigned)
  ...
  const showCustomerSelector = !customerId;
            ...
            <TabsContent value="sequence" className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {selectedBrand?.default_visit_sequence?.length > 0 && (
                  <Button type="button" variant="outline" size="sm" onClick={loadBrandDefaults}>
                    <RefreshCw className="h-4 w-4 ml-2" />
                    טען מברירת מחדל ({selectedBrand.name})
                  </Button>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                ...
                />
                <span className="text-sm text-blue-600">חודשים (ישמש כשאין אינטרוול ספציפי לשלב)</span>
              </div>
